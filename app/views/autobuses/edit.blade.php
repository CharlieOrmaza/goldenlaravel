...
<script type="text/javascript">
	...
	<div class="panel panel-success">
  		<div class="panel-heading">
  			<h4>Actualizar Autobus</h4>
  		</div>
                 
  		<div class="panel-body">
  			@if (!empty($autobuses))
    			<form method="post" action="/autobuses/update/{{ $autobuses->id }}">
		            <p>
						<h3>Papeleta</h3><hr>
					</p>
		          	<p>
				    ...
				    <table class="table">
                    	...
                    	<tbody>
				        	...
				        	<tr>
				        		<th>
				        			<input  value="{{ $autobuses->operador  }}" type="text" name="ope"  placeholder="Operador" class="form-control" required>
				        		</th>
				        		<th>
				      				<input value="{{ $autobuses->linea}}" type="text" name="linea" placeholder="Linea" class="form-control" required>
				      			</th>
				      			<th>
				      				<input value="{{ $autobuses->tipo  }}" type="text" placeholder="Tipo de Viaje" name="tipo"  class="form-control" required>
				      			</th>
				        	</tr>
				        	<tr>
				      			<th>
				        	...
				        	<tr>
				        		<th>
				        			<input value="{{ $autobuses->fechaSalida }}" type="date" name="fechaS"  class="form-control" required>
				        		</th>
				        		<th>
				      				<input value="{{ $autobuses->fechaRegreso}}" type="date" name="fechaR"  class="form-control" required>
				      			</th>
				      			<th>
				      				<input value="{{ $autobuses->numeroPersonas }}" type="number" placeholder="Numero de Personas" name="numeroPersonas"  class="form-control" required>
				      			</th>
				        	</tr>
				        <tbody>
                    </table>
		          	<p>
		            	Origen y Horario: 
		          	    <textarea  name="origenyHorario" placeholder="Origen y Horario" class="form-control" >{{ $autobuses->origenyHorario }}</textarea>
		          	</p>
		          	<p>
		            	Destino y Horario:: 
		          	    <textarea  name="destinoyHorario" placeholder="Destino y Horario" class="form-control" >{{ $autobuses->destinoyHorario }}</textarea>
		          	</p>
		          	<hr>
	  				<p>
